...
import lp_fa4.cute.utils as utils
from lp_fa4.cute.fast_math import clz
...
class PersistentVarlenLookupScheduler:
    """Persistent varlen scheduler backed by host-precomputed coordinates."""
    ...
    class Params(ParamsBase):
        ...
        def create(
            args: TileSchedulerArguments, *, loc=None, ip=None
        ) -> "PersistentVarlenLookupScheduler.Params":
            assert args.mTileToBatch is not None
            assert args.mTileToHead is not None
            assert args.mTileToBlock is not None
            return PersistentVarlenLookupScheduler.Params(
                args.num_batch,
                args.mTileToBatch,
                args.mTileToHead,
                args.mTileToBlock,
    ...
    def to_underlying_arguments(
        ...
        *,
        ...
        assert scheduling_mode == SchedulingMode.STATIC
        return PersistentVarlenLookupScheduler.Params.create(args, loc=loc, ip=ip)
    ...
    def create(
        ...
        return PersistentVarlenLookupScheduler(
    ...
    def get_grid_shape(
        ...
        *,
        ...
        return (
            cutlass.min(sm_count, params.mTileToBatch.shape[0]),
            Int32(1),
            Int32(1),
        )
    ...
    @cute.jit
    def get_current_work(self, *, loc=None, ip=None) -> WorkTileInfo:
        block, head_idx, batch_idx = Int32(0), Int32(0), self.params.num_batch
        is_valid = False
        if self._tile_idx < self.params.mTileToBatch.shape[0]:
            batch_idx = self.params.mTileToBatch[self._tile_idx]
            head_idx = self.params.mTileToHead[self._tile_idx]
            block = self.params.mTileToBlock[self._tile_idx]
            is_valid = batch_idx < self.params.num_batch
        return WorkTileInfo(
            (block, head_idx, batch_idx, Int32(0)),
            is_valid,
        )
...
class SingleTileVarlenScheduler:
        ...
        def create(
            ...
            *,
            ...
            return SingleTileVarlenScheduler.Params(
                ...
                broadcast_q_seqlen=args.broadcast_q_seqlen,
                is_persistent=args.is_persistent
                and scheduling_mode == SchedulingMode.STATIC,
                scheduling_mode=scheduling_mode,
            )
    ...
    def get_grid_shape(
        ...
        *,
        ...
        if cutlass.const_expr(params.broadcast_q):
            num_m_blocks = cute.ceil_div(
                cute.ceil_div(params.broadcast_q_seqlen, params.tile_shape_mn[0]),
                params.cluster_shape_m,
            )
            total_blocks_max = (
                num_m_blocks * params.cluster_shape_m * params.num_batch
            )
        else:
            total_blocks_max = (
                params.total_q
        ...
        if cutlass.const_expr(params.is_persistent):
            ...
            sm_count = hardware_info.get_device_multiprocessor_count()
            max_ctas = sm_count // params.cluster_shape_m * params.cluster_shape_m
            grid_x = cutlass.min(max_ctas, grid_x)
    ...
    def _get_num_m_blocks(self, lane: Int32, bidb_start: Int32) -> Int32:
        ...
        if cutlass.const_expr(params.broadcast_q):
            seqlen = params.broadcast_q_seqlen
    ...
    def _varlen_coord_map(self) -> WorkTileInfo:
        ...
        params = self.params
        if cutlass.const_expr(params.broadcast_q):
            broadcast_num_m_blocks = cute.ceil_div(
                cute.ceil_div(params.broadcast_q_seqlen, params.tile_shape_mn[0]),
                params.cluster_shape_m,
            )
            broadcast_tiles_per_batch = broadcast_num_m_blocks * params.num_head
            broadcast_tile_idx = self._tile_idx
            if cutlass.const_expr(not params.use_cluster_idx):
                broadcast_tile_idx = broadcast_tile_idx // params.cluster_shape_m
            broadcast_batch_idx = broadcast_tile_idx // broadcast_tiles_per_batch
            broadcast_mh_block = (
                broadcast_tile_idx
                - broadcast_batch_idx * broadcast_tiles_per_batch
            )
            broadcast_head_idx = broadcast_mh_block // broadcast_num_m_blocks
            broadcast_block = (
                broadcast_mh_block
                - broadcast_head_idx * broadcast_num_m_blocks
            )
            broadcast_is_valid = broadcast_batch_idx < params.num_batch
            if cutlass.const_expr(not params.is_persistent):
                broadcast_is_valid = self._is_first_block and broadcast_is_valid
            if cutlass.const_expr(
                params.cluster_shape_m > 1 and not params.use_cluster_idx
            ):
                bidx_in_cluster = cute.arch.block_in_cluster_idx()
                broadcast_block = (
                    broadcast_block * params.cluster_shape_m + bidx_in_cluster[0]
                )
            broadcast_split_idx = (
                self._split_idx if const_expr(params.is_split_kv) else Int32(0)
            )
            return WorkTileInfo(
                (
                    Int32(broadcast_block),
                    Int32(broadcast_head_idx),
                    Int32(broadcast_batch_idx),
                    broadcast_split_idx,
                ),
                broadcast_is_valid,
            )
        ...
        if cutlass.const_expr(not params.use_cluster_idx):
            next_tile_idx = next_tile_idx // params.cluster_shape_m
...
class PersistentVarlenTileScheduler(SingleTileVarlenScheduler):
    @staticmethod
    @cute.jit
    def create(
        ...
        *,
        ...
        assert params.scheduling_mode == SchedulingMode.STATIC
        tile_idx, split_idx, _ = cute.arch.block_idx()
        return PersistentVarlenTileScheduler(
            params,
            tile_idx,
            split_idx,
            loc=loc,
    ...
    def get_grid_shape(
        ...
        *,
        ...
        total_grid = SingleTileVarlenScheduler.get_grid_shape(params)
        sm_count = HardwareInfo().get_device_multiprocessor_count()
        return (
            cutlass.min(sm_count, total_grid[0]),
            total_grid[1],
            total_grid[2],
        )
