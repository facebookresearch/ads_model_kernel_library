...
import lp_fa4.cute.mma_sm100_desc as sm100_desc


_MMA_FP8_TYPES = tuple(
    mma_type
    for mma_type in (
...
def _tcgen05_mma_kind(op: cute.nvgpu.tcgen05.mma.MmaOp) -> str:
    ...
    if isinstance(op, _MMA_FP8_TYPES):
        return "f8f6f4"
...
def make_s2t_copy_partitions(
    ...
    copy_atom = cute.make_copy_atom(tcgen05.Cp4x32x128bOp(cta_group), sf_dtype)
...
def gemm_blockscaled(
    ...
    for kphase_idx in cutlass.range(num_kphases, unroll_full=True):
        ...
        sfa_iter = tCtSFA[kphase_coord].iterator
        sfb_iter = tCtSFB[kphase_coord].iterator
        tiled_mma.set(tcgen05.Field.SFA, sfa_iter)
        tiled_mma.set(tcgen05.Field.SFB, sfb_iter)
        if kphase_idx == 0:
            tiled_mma.set(tcgen05.Field.ACCUMULATE, not zero_init)
        else:
            tiled_mma.set(tcgen05.Field.ACCUMULATE, True)
        cute.gemm(tiled_mma, acc, tCrA[kphase_coord], tCrB[kphase_coord], acc)
