...
def call_mask_mod(
    ...
    aux_tensors = aux_data.tensors if aux_data is not None else None
    # Compatibility shim for pre-aux_scalars mask_mod callables.
    if const_expr(aux_data is not None and aux_data.scalars is not None):
        return mask_mod(
            batch_idx,
            head_idx,
            q_idx,
    ...
    def apply_mask(
        ...
        if const_expr(not mask_causal and not mask_local and mask_mod is None):
            ...
            wrap_aux_indices = const_expr(
                has_fastdiv
                and mask_seqlen
                and const_expr(aux_data is not None and aux_data.tensors is not None)
            )
