...
def fused_amax_to_e8m0_scale_f32(
    ...
    *,
    ...
    result = llvm.inline_asm(
        ...
        "{\n"
        ".reg .f32 scale;\n"
        ".reg .u32 bits, exp, mantissa;\n"
        ".reg .pred has_mantissa;\n"
        "mul.f32 scale, $2, $3;\n"
        "mov.b32 bits, scale;\n"
        "bfe.u32 exp, bits, 23, 8;\n"
        "and.b32 mantissa, bits, 0x7FFFFF;\n"
        "setp.ne.u32 has_mantissa, mantissa, 0;\n"
        "@has_mantissa add.u32 exp, exp, 1;\n"
        "min.u32 exp, exp, 254;\n"
        "mov.b32 $0, exp;\n"
        ".reg .u32 inv_exp, inv_bits;\n"
        "sub.u32 inv_exp, 254, exp;\n"
        "shl.b32 inv_bits, inv_exp, 23;\n"
        "mov.b32 $1, inv_bits;\n"
        "}\n",
        "=r,=f,f,f",
        has_side_effects=False,
    ...
    inv_scale = llvm.extractvalue(T.f32(), result, [1], loc=loc, ip=ip)
    return Float32(inv_scale), cutlass.Uint32(e8m0_u32)


@dsl_user_op
def fused_amax_to_e8m0_scale_f32_hw(
    amax: Float32,
    max_norm_rcp: Float32,
    *,
    loc=None,
    ...
    result = llvm.inline_asm(
        ...
        ".reg .u32 packed, exp;\n"
        "mul.f32 scale, $2, $3;\n"
        "cvt.rp.satfinite.ue8m0x2.f32 e8m0x2, scale, scale;\n"
        "cvt.u32.u16 packed, e8m0x2;\n"
        "and.b32 exp, packed, 0xFF;\n"
        "min.u32 exp, exp, 254;\n"
        "mov.b32 $0, exp;\n"
        ".reg .u32 inv_exp, inv_bits;\n"
        "sub.u32 inv_exp, 254, exp;\n"
...
def call_score_mod(
    ...
    aux_tensors = (
        aux_data.tensors
        if aux_data is not None and aux_data.tensors is not None
        else ()
    )
    # Compatibility shim for pre-aux_scalars score_mod callables.
    if cutlass.const_expr(aux_data is not None and aux_data.scalars is not None):
        return score_mod(
            score,
            batch_idx,
            head_idx,
...
class SoftmaxSm100(Softmax):
    ...
    def update_row_max_precomputed(
        self, row_max_current: Float32, is_first: int
    ) -> Tuple[Float32, Float32]:
        if cutlass.const_expr(is_first):
            row_max_new = row_max_current
            row_max_safe = (
                row_max_new if row_max_new != -cutlass.Float32.inf else 0.0
            )
            acc_scale = 0.0
        else:
            row_max_old = self.row_max[0]
            row_max_new = cute.arch.fmax(row_max_current, row_max_old)
            row_max_safe = (
                row_max_new if row_max_new != -cutlass.Float32.inf else 0.0
            )
            acc_scale_ = (row_max_old - row_max_safe) * self.scale_log2
    ...
    def apply_exp2_convert(
        ...
        for j in cutlass.range_constexpr(frg_cnt):
            ...
            if cutlass.const_expr(acc_S_row_converted.element_type.width == 8):
                utils.cvt_fp8(
                    acc_S_row_frg[None, j], acc_S_row_converted_frg[None, j]
                )
            else:
                acc_S_row_converted_frg[None, j].store(
                    acc_S_row_frg[None, j]
                    .load()
                    .to(acc_S_row_converted.element_type)
                )
...
def apply_score_mod_inner(
    ...
    Args:
        ...
        aux_data: Optional tensor and scalar captures for FlexAttention
    ...
    for i in cutlass.range(0, n_vals, vec_size, unroll_full=True):
        for j in cutlass.range(vec_size, unroll_full=True):
            ...
            if cutlass.const_expr(
                ...
                and aux_data.tensors is not None
                and fastdiv_mods is not None
            ):
