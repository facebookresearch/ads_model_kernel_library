...
from lp_fa4.cute import ampere_helpers as sm80_utils
...
class FlashAttentionBackwardPostprocess:
    ...
    def _setup_attributes(self):
        ...
        async_copy_elems_accum = universal_copy_bits // self.dqaccum_dtype.width
        ...
        if const_expr(self.arch // 10 in [8, 12]):
            self.s2r_tiled_copy_dQaccum = copy_utils.tiled_copy_1d(
                self.dqaccum_dtype, self.num_threads, num_s2r_copy_elems
            )
            ...
            self.s2r_tiled_copy_dQaccum = cute.make_tiled_copy_tv(
                ...
                cute.make_layout(128 // self.dqaccum_dtype.width),  # val_layout
            )
    ...
    def __call__(
        ...
        if const_expr(mdQaccum is not None):
            if const_expr(
                mdQaccum.element_type not in [cutlass.Float32, cutlass.Float16]
            ):
    ...
    def kernel(
        ...
        sdQaccum = smem.allocate_tensor(
            self.dqaccum_dtype, sdQaccum_layout, byte_alignment=1024
        )
        ...
        if work_tile.is_valid_tile:
            ...
            if const_expr(self.arch // 10 in [10, 11] and self.use_2cta_instrs):
                ...
                # S -> R
                tdQrdQ_accum = cute.make_rmem_tensor(
                    tdQrdQ.shape, self.dqaccum_dtype
                )
                tdQrdQ_s2r = cute.make_tensor(
                    tdQrdQ_accum.iterator, tdQrdQ_accum.shape
                )
                ...
                num_stages = cute.size(tdQrdQ_accum, mode=[1])
                ...
                for stage_group in cutlass.range_constexpr(stage_groups):
                    ...
                    for stage_offset, lane_offset in stage_iters:
                        ...
                        dQ_vec = tdQrdQ_r2s_cpy.load().to(Float32) * scale
                ...
                if const_expr(self.arch // 10 in [8, 9, 12]):
                    ...
                    acc = cute.make_rmem_tensor(acc_shape, self.dqaccum_dtype)
