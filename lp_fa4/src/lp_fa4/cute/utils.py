...
def _cutlass_major_minor(version: str) -> tuple[int, int]:
    release = Version(version).release
    return release[0], release[1] if len(release) > 1 else 0


_TMA_COPY_NEEDS_ELECT_ONE = _cutlass_major_minor(cutlass.__version__) < (4, 6)


def copy_bulk_g2s_maybe_elect(copy_atom, *args, **kwargs):
    """Issue a bulk G2S copy with version-correct election."""
    if const_expr(_TMA_COPY_NEEDS_ELECT_ONE):
        with cute.arch.elect_one():
            cute.copy(copy_atom, *args, **kwargs)
    else:
        cute.copy(copy_atom, *args, **kwargs)
...
def cvt_fp8x4_f32(
    ...
    *,
    ...
    assert to_dtype in [cutlass.Float8E4M3FN, cutlass.Float8E5M2]
...
@cute.jit
def cvt_fp8(src: cute.Tensor, dst: cute.Tensor) -> None:
    """Convert a Float32 register tensor to E4M3 or E5M2."""
    ...
    assert dst.element_type in [cutlass.Float8E4M3FN, cutlass.Float8E5M2]
    assert cute.size(src.shape) % 4 == 0, (
        "src must have a multiple of 4 elements"
    )
