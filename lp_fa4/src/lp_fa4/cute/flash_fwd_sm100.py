...
from lp_fa4.cute.pack_gqa import PackGQA, pack_gqa_layout
from lp_fa4.cute import mma_sm100_desc as sm100_desc
from lp_fa4.cute import blackwell_helpers as sm100_utils
from lp_fa4.cute.named_barrier import NamedBarrierFwdSm100
...
from lp_fa4.cute.fa_logging import fa_log, fa_printf
from lp_fa4.cute.utils import smid
...
class FlashAttentionForwardSm100:
    ...
    def __init__(
        ...
        external_mx: cutlass.Constexpr[bool] = False,
        broadcast_q: cutlass.Constexpr[bool] = False,
        unroll_external_mx: cutlass.Constexpr[bool] = False,
        use_jagged_fwd_lookup: cutlass.Constexpr[bool] = False,
    ):
        ...
        self.external_mx = external_mx
        self.broadcast_q = broadcast_q
        self.use_jagged_fwd_lookup = use_jagged_fwd_lookup
        assert not unroll_external_mx or (external_mx and broadcast_q)
        assert not use_jagged_fwd_lookup or (
            external_mx
            and broadcast_q
            and is_varlen_q
            and is_persistent
        ...
        assert not external_mx or blockscaled
        if blockscaled:
            assert not self.use_2cta_instrs
            assert self.head_dim_padded == self.head_dim_v_padded == 128
        ...
        self.use_tma_O = (
            ...
            and (not is_varlen_q or self.ragged_tma_O)
        )
        ...
        self.unroll_kv = (
            ...
            and not self.is_split_kv
            and (not self.external_mx or unroll_external_mx)
        )

        if self.use_jagged_fwd_lookup:
            self.TileScheduler = PersistentVarlenLookupScheduler
        elif is_varlen_q:
        ...
        if self.blockscaled:
            self.tmem_total = self.tmem_alloc_cols
        assert self.tmem_total <= self.tmem_alloc_cols
        self.tmem_s_to_p_offset = (
            self.n_block_size // 4
            if self.blockscaled and self.q_stage == 2
            else self.n_block_size // 2
        )
        self.tmem_p_offset = [
        ...
        self.mbar_mma_p0_id = int(NamedBarrierFwdSm100.MmaP0)
        self.mbar_mma_p1_id = int(NamedBarrierFwdSm100.MmaP1)
    ...
    @cute.jit
    def _fill_external_sfp(self, tensor: cute.Tensor):
        ...
        for group in cutlass.range_constexpr(
            cute.ceil_div(num_words, self.threads_per_cta)
        ):
            word_idx = tidx + group * self.threads_per_cta
            if word_idx < num_words:
                addr = Int32(utils.elem_pointer(tensor_u32, (word_idx,)).toint())
                _st_shared_b32(addr, Int32(0x77777777))
    ...
    def _setup_attributes(self):
        ...
        if self.blockscaled:
            ...
            kv_stage = min(kv_stage, max_kv_stage)
    ...
    def __call__(
        ...
        total_sf_v: Int32 | int | None = None,
        ...
        if const_expr(self.blockscaled):
            sfq_stages = self.q_stage if const_expr(self.external_mx) else 1
            sfkv_stages = self.kv_stage if const_expr(self.external_mx) else 1
            sSFQ_layout = blockscaled_utils.make_smem_layout_sfa(
                tiled_mma_qk, self.mma_tiler_qk, self.sf_vec_size, sfq_stages
            )
            sSFK_layout = blockscaled_utils.make_smem_layout_sfb(
                tiled_mma_qk, self.mma_tiler_qk, self.sf_vec_size, sfkv_stages
            )
            sSFP_layout = blockscaled_utils.make_smem_layout_sfa(
                tiled_mma_pv, self.mma_tiler_pv, self.sf_vec_size, 1
            )
            sSFV_layout = blockscaled_utils.make_smem_layout_sfb(
                tiled_mma_pv, self.mma_tiler_pv, self.sf_vec_size, sfkv_stages
            )
        else:
            sSFQ_layout = None
            sSFK_layout = None
        ...
        if const_expr(self.external_mx):
            ...
            self.tma_copy_bytes["V"] += cute.size_in_bytes(
        ...
        gmem_shape_Q = mQ.shape
        gmem_shape_K = mK.shape
        gmem_shape_V = mV.shape
        ...
        tma_atom_SFQ = tma_atom_SFK = tma_atom_SFV = None
        tma_tensor_SFQ = tma_tensor_SFK = tma_tensor_SFV = None
        if const_expr(self.external_mx):
            assert mSFQ is not None and mSFK is not None and mSFV is not None
            assert total_sf_q is not None and total_sf_k is not None and total_sf_v is not None
            sfq_layout = blockscaled_utils.tile_atom_to_shape_SF(
                (total_sf_q, self.head_dim_padded, gmem_shape_Q[2]),
                self.sf_vec_size,
            )
            sfk_layout = blockscaled_utils.tile_atom_to_shape_SF(
                (total_sf_k, self.head_dim_padded, gmem_shape_K[2]),
                self.sf_vec_size,
            )
            sfv_layout = blockscaled_utils.tile_atom_to_shape_SF(
                (self.head_dim_v_padded, total_sf_v, gmem_shape_V[2]),
                self.sf_vec_size,
            )
        ...
        if const_expr(self.use_tma_O):
            if const_expr(self.ragged_tma_O):
                mO = copy_utils.create_ragged_tensor_for_tma(
                    mO, ragged_dim=0, ptr_shift=True
                )
        ...
        num_q_blocks = cute.ceil_div(cute.size(mQ.shape[0]), _num_block_divisor)
        tile_sched_args = TileSchedulerArguments(
            ...
            and (not self.is_persistent or self.is_varlen_q),
            broadcast_q=self.broadcast_q,
            broadcast_q_seqlen=mQ.shape[0] if const_expr(self.broadcast_q) else 0,
        ...
        sSFQ_size = cute.cosize(sSFQ_layout) if const_expr(self.blockscaled) else 0
        sSFK_size = cute.cosize(sSFK_layout) if const_expr(self.blockscaled) else 0
        sSFP_size = cute.cosize(sSFP_layout) if const_expr(self.blockscaled) else 0
        sSFV_size = cute.cosize(sSFV_layout) if const_expr(self.blockscaled) else 0
        ...
        class SharedStorage:
            ...
            sSFQ: cute.struct.Align[
                cute.struct.MemRange[sf_dtype_alloc, sSFQ_size], 128
            ]
            sSFK: cute.struct.Align[
                cute.struct.MemRange[sf_dtype_alloc, sSFK_size], 128
            ]
            sSFP: cute.struct.Align[
                cute.struct.MemRange[sf_dtype_alloc, sSFP_size], 128
            ]
            sSFV: cute.struct.Align[
                cute.struct.MemRange[sf_dtype_alloc, sSFV_size], 128
            ]
        ...
        aux_tensors = aux_data.tensors if const_expr(aux_data is not None) else None
    ...
    def kernel(
        ...
        if const_expr(self.blockscaled):
            ...
            assert self.external_mx
            self._fill_external_sfp(sSFP)
    ...
    def load(
        ...
        while work_tile.is_valid_tile:
            ...
            if const_expr(self.external_mx):
                assert mSFQ is not None and mSFK is not None and mSFV is not None
                assert sSFQ is not None and sSFK is not None and sSFV is not None
                ...
                tSgSFV = thr_mma_pv.partition_B(gSFV)
                ...
                load_Q = partial(
                    ...
                    tSFQgSFQ=tSFQgSFQ,
                    tSFQsSFQ=tSFQsSFQ,
                    sf_block_base=self.q_stage * m_block,
                )
    ...
    def mma(
        ...
        tCtSFQ = tCtSFK = tCtSFP = tCtSFV = None
        tCtSFQs = [None, None]
        tCtSFKs = [None, None]
        tCtSFPs = [None, None]
        tCtSFVs = [None, None]
        ...
        tCtSFQ_compacts = [None, None]
        tCtSFK_compacts = [None, None]
        tCtSFP_compacts = [None, None]
        tCtSFV_compacts = [None, None]
        tCtSFQ_compacts_prologue = [None, None]
        tCtSFK_compacts_prologue = [None, None]
        if const_expr(self.blockscaled):
            tmem_ptr = cute.make_ptr(
                Float32, 0, mem_space=cute.AddressSpace.tmem, assumed_align=16
            )
            sSFQ_stage = cute.slice_(sSFQ.layout, (None, None, None, 0))
            sSFK_stage = cute.slice_(sSFK.layout, (None, None, None, 0))
            sSFP_stage = cute.slice_(sSFP.layout, (None, None, None, 0))
            sSFV_stage = cute.slice_(sSFV.layout, (None, None, None, 0))
            tCtSFQ_layout = blockscaled_utils.make_tmem_layout_sfa(
                tiled_mma_qk, self.mma_tiler_qk, self.sf_vec_size, sSFQ_stage
            )
            tCtSFK_layout = blockscaled_utils.make_tmem_layout_sfb(
                tiled_mma_qk, self.mma_tiler_qk, self.sf_vec_size, sSFK_stage
            )
            tCtSFP_layout = blockscaled_utils.make_tmem_layout_sfa(
                tiled_mma_pv, self.mma_tiler_pv, self.sf_vec_size, sSFP_stage
            )
            tCtSFV_layout = blockscaled_utils.make_tmem_layout_sfb(
                tiled_mma_pv, self.mma_tiler_pv, self.sf_vec_size, sSFV_stage
            )

            if const_expr(self.q_stage == 1):
                qk_sf_base = 384
                tCtSFQ = cute.make_tensor(
                    cute.recast_ptr(tmem_ptr + qk_sf_base, dtype=self.sf_dtype),
                    tCtSFQ_layout,
                )
                sfk_offset = tcgen05.find_tmem_tensor_col_offset(tCtSFQ)
                tCtSFK = cute.make_tensor(
                    cute.recast_ptr(
                        tmem_ptr + qk_sf_base + sfk_offset, dtype=self.sf_dtype
                    ),
                    tCtSFK_layout,
                )
                pv_sf_base = 448
                tCtSFP = cute.make_tensor(
                    cute.recast_ptr(tmem_ptr + pv_sf_base, dtype=self.sf_dtype),
                    tCtSFP_layout,
                )
                sfv_offset = tcgen05.find_tmem_tensor_col_offset(tCtSFP)
                tCtSFV = cute.make_tensor(
                    cute.recast_ptr(
                        tmem_ptr + pv_sf_base + sfv_offset, dtype=self.sf_dtype
                    ),
                    tCtSFV_layout,
                )
                tiled_copy_sfq, sSFQ_compact, tCtSFQ_compact = (
                    sm100_utils.make_s2t_copy_partitions(sSFQ, tCtSFQ, self.sf_dtype)
                )
                tiled_copy_sfk, sSFK_compact, tCtSFK_compact = (
                    sm100_utils.make_s2t_copy_partitions(sSFK, tCtSFK, self.sf_dtype)
                )
                tiled_copy_sfp, sSFP_compact, tCtSFP_compact = (
                    sm100_utils.make_s2t_copy_partitions(sSFP, tCtSFP, self.sf_dtype)
                )
                tiled_copy_sfv, sSFV_compact, tCtSFV_compact = (
                    sm100_utils.make_s2t_copy_partitions(sSFV, tCtSFV, self.sf_dtype)
                )
                sf_stage_coord = (None, None, None, None, 0)
                if const_expr(not self.external_mx):
                    cute.copy(tiled_copy_sfq, sSFQ_compact[sf_stage_coord], tCtSFQ_compact)
                    cute.copy(tiled_copy_sfk, sSFK_compact[sf_stage_coord], tCtSFK_compact)
                    cute.copy(tiled_copy_sfp, sSFP_compact[sf_stage_coord], tCtSFP_compact)
                    cute.copy(tiled_copy_sfv, sSFV_compact[sf_stage_coord], tCtSFV_compact)
                    cute.arch.fence_view_async_tmem_store()
            else:
                # Two Q stages consume all 512 TMEM columns. QK scales use the
                # opposite stage's drained S tail, PV scales follow P, and the
                # first QK pair temporarily uses O before O becomes live.
                sfk_offset = tcgen05.find_tmem_tensor_col_offset(
                    cute.make_tensor(
                        cute.recast_ptr(tmem_ptr, dtype=self.sf_dtype), tCtSFQ_layout
                    )
                )
                sfv_offset = tcgen05.find_tmem_tensor_col_offset(
                    cute.make_tensor(
                        cute.recast_ptr(tmem_ptr, dtype=self.sf_dtype), tCtSFP_layout
                    )
                )
                tile_p_cols = self.n_block_size // 32 * self.v_dtype.width
                qk_sf_bases = (
                    self.tmem_p_offset[1] + tile_p_cols * 2,
                    self.tmem_p_offset[0] + tile_p_cols * 2,
                )
                pv_sf_bases = (
                    self.tmem_p_offset[0] + tile_p_cols,
                    self.tmem_p_offset[1] + tile_p_cols,
                )
                qk_prologue_bases = (self.tmem_o_offset[1], self.tmem_o_offset[0])
                for stage in cutlass.range_constexpr(2):
                    tCtSFQs[stage] = cute.make_tensor(
                        cute.recast_ptr(
                            tmem_ptr + qk_sf_bases[stage], dtype=self.sf_dtype
                        ),
                        tCtSFQ_layout,
                    )
                    tCtSFKs[stage] = cute.make_tensor(
                        cute.recast_ptr(
                            tmem_ptr + qk_sf_bases[stage] + sfk_offset,
                            dtype=self.sf_dtype,
                        ),
                        tCtSFK_layout,
                    )
                    tCtSFPs[stage] = cute.make_tensor(
                        cute.recast_ptr(
                            tmem_ptr + pv_sf_bases[stage], dtype=self.sf_dtype
                        ),
                        tCtSFP_layout,
                    )
                    tCtSFVs[stage] = cute.make_tensor(
                        cute.recast_ptr(
                            tmem_ptr + pv_sf_bases[stage] + sfv_offset,
                            dtype=self.sf_dtype,
                        ),
                        tCtSFV_layout,
                    )
                    tCtSFQs_prologue[stage] = cute.make_tensor(
                        cute.recast_ptr(
                            tmem_ptr + qk_prologue_bases[stage], dtype=self.sf_dtype
                        ),
                        tCtSFQ_layout,
                    )
                    tCtSFKs_prologue[stage] = cute.make_tensor(
                        cute.recast_ptr(
                            tmem_ptr + qk_prologue_bases[stage] + sfk_offset,
                            dtype=self.sf_dtype,
                        ),
                        tCtSFK_layout,
                    )
                tiled_copy_sfq, sSFQ_compact, tCtSFQ_compacts[0] = (
                    sm100_utils.make_s2t_copy_partitions(
                        sSFQ, tCtSFQs[0], self.sf_dtype
                    )
                )
                tiled_copy_sfk, sSFK_compact, tCtSFK_compacts[0] = (
                    sm100_utils.make_s2t_copy_partitions(
                        sSFK, tCtSFKs[0], self.sf_dtype
                    )
                )
                tiled_copy_sfp, sSFP_compact, tCtSFP_compacts[0] = (
                    sm100_utils.make_s2t_copy_partitions(
                        sSFP, tCtSFPs[0], self.sf_dtype
                    )
                )
                tiled_copy_sfv, sSFV_compact, tCtSFV_compacts[0] = (
                    sm100_utils.make_s2t_copy_partitions(
                ...
                for stage in cutlass.range_constexpr(2):
                    if const_expr(stage == 1):
                        _, _, tCtSFQ_compacts[stage] = (
                            sm100_utils.make_s2t_copy_partitions(
                                sSFQ, tCtSFQs[stage], self.sf_dtype
                            )
                        )
                        _, _, tCtSFK_compacts[stage] = (
                            sm100_utils.make_s2t_copy_partitions(
                                sSFK, tCtSFKs[stage], self.sf_dtype
                            )
                        )
                        _, _, tCtSFP_compacts[stage] = (
                            sm100_utils.make_s2t_copy_partitions(
                                sSFP, tCtSFPs[stage], self.sf_dtype
                            )
                        )
                        _, _, tCtSFV_compacts[stage] = (
                            sm100_utils.make_s2t_copy_partitions(
                                sSFV, tCtSFVs[stage], self.sf_dtype
                            )
                        )
                    _, _, tCtSFQ_compacts_prologue[stage] = (
                        sm100_utils.make_s2t_copy_partitions(
                            sSFQ, tCtSFQs_prologue[stage], self.sf_dtype
                        )
                    )
                    _, _, tCtSFK_compacts_prologue[stage] = (
                        sm100_utils.make_s2t_copy_partitions(
                            sSFK, tCtSFKs_prologue[stage], self.sf_dtype
                        )
                    )
        ...
        while work_tile.is_valid_tile:
            ...
            if process_tile and is_leader_cta and (
                is_first_tile or const_expr(not self.unroll_kv)
            ):
                for stage in cutlass.range_constexpr(self.q_stage):
                    ...
                    if const_expr(self.blockscaled):
                        if const_expr(self.q_stage == 1):
                            if const_expr(self.external_mx):
                                sfq_coord = (None, None, None, None, stage)
                                sfk_coord = (None, None, None, None, Ki_index)
                                cute.copy(
                                    tiled_copy_sfq,
                                    sSFQ_compact[sfq_coord],
                                    tCtSFQ_compact,
                                )
                                cute.copy(
                                    tiled_copy_sfk,
                                    sSFK_compact[sfk_coord],
                                    tCtSFK_compact,
                                )
                                cute.arch.fence_view_async_tmem_store()
                            ...
                            sfq_coord = (
                                ...
                                None,
                                None,
                                stage if const_expr(self.external_mx) else 0,
                            )
                            sfk_coord = (
                                None,
                                None,
                                None,
                                None,
                                Ki_index if const_expr(self.external_mx) else 0,
                            )
                            if is_first_tile:
                                cute.copy(
                                    tiled_copy_sfq,
                                    sSFQ_compact[sfq_coord],
                                    tCtSFQ_compacts_prologue[stage],
                                )
                                cute.copy(
                                    tiled_copy_sfk,
                                    sSFK_compact[sfk_coord],
                                    tCtSFK_compacts_prologue[stage],
                                )
                                cute.arch.fence_view_async_tmem_store()
                                sm100_utils.gemm_blockscaled(
                                    tiled_mma_qk,
                                ...
                                cute.copy(
                                    tiled_copy_sfq,
                                    sSFQ_compact[sfq_coord],
                                    tCtSFQ_compacts[stage],
                                )
                                cute.copy(
                                    tiled_copy_sfk,
                                    sSFK_compact[sfk_coord],
                                    tCtSFK_compacts[stage],
                                )
                                cute.arch.fence_view_async_tmem_store()
                                sm100_utils.gemm_blockscaled(
                                    tiled_mma_qk,
            ...
            if process_tile and is_leader_cta:
                    ...
                    for stage in cutlass.range_constexpr(self.q_stage):
                        ...
                        if const_expr(self.blockscaled):
                            sfv_coord = (
                                ...
                                None,
                                None,
                                Vi_index if const_expr(self.external_mx) else 0,
                            )
                            if const_expr(self.q_stage == 2):
                                cute.copy(
                                    tiled_copy_sfp,
                                    sSFP_compact[sf_stage_coord],
                                    tCtSFP_compacts[stage],
                                )
                                cute.copy(
                                    tiled_copy_sfv,
                                    sSFV_compact[sfv_coord],
                                    tCtSFV_compacts[stage],
                                )
                                cute.arch.fence_view_async_tmem_store()
                                if const_expr(self.split_P_arrive > 0):
                                    pipeline_p_lastsplit.sync_object_full.wait(
                                        stage, P_full_O_rescaled_phase
                                    )
                                sfp_tensor = tCtSFPs[stage]
                                sfv_tensor = tCtSFVs[stage]
                            else:
                                if const_expr(self.external_mx):
                                    cute.copy(
                                        tiled_copy_sfp,
                        ...
                        if const_expr(self.blockscaled):
                            ...
                            if const_expr(self.q_stage == 2):
                                ...
                                cute.arch.fence_view_async_tmem_store()
                                sfq_tensor = tCtSFQs[stage]
                                sfk_tensor = tCtSFKs[stage]
                            else:
                                if const_expr(self.external_mx):
                                    cute.copy(
                                        tiled_copy_sfq,
                        ...
                        pipeline_s_p_o.producer_commit_w_index(stage)
                        if const_expr(self.external_mx):
                            # QK and the next PV reuse adjacent TMEM scale storage.
                            pipeline_s_p_o.sync_object_full.wait(
                                stage, s_full_producer_phase
                            )
                ...
                if const_expr(not self.unroll_kv):
                    ...
                    mma_kv_release_state = mma_kv_consumer_state.clone()
                    sV_cur = sV[None, None, None, Vi_index]
                    sfv_coord = sf_stage_coord
                    sfq_coord = sf_stage_coord
                    sfk_coord = sf_stage_coord
                    if is_next_tile_valid:
                        ...
                        pipeline_kv.consumer_release(mma_kv_consumer_state)
                        mma_kv_consumer_state.advance()
            if not process_tile or not is_leader_cta:
                work_tile = tile_scheduler.advance_to_next_work()
        # End of persistent scheduler loop
    ...
    def softmax_loop(
        ...
        if const_expr(
            ...
            tmem_load_red = tcgen05.make_tmem_copy(tmem_load_red_atom, tSAcc)
            thr_tmem_load_red = tmem_load_red.get_slice(tidx)
            tStS_t2r_red = thr_tmem_load_red.partition_S(tSAcc)
            tScS_t2r_red = thr_tmem_load_red.partition_D(tScS)
        ...
        while work_tile.is_valid_tile:
            ...
            # P uses a fixed 2^-8 scale, so deferred rescaling must keep exp2
            # values within the output dtype's finite range.
            if const_expr(self.q_dtype.width == 16):
                rescale_threshold = 8.0
            elif const_expr(self.q_dtype is cutlass.Float8E4M3FN):
                rescale_threshold = math.log2(448.0 / 256.0)
            elif const_expr(self.q_dtype is cutlass.Float8E5M2):
                rescale_threshold = math.log2(57344.0 / 256.0)
            else:
            ...
            softmax_step = partial(
                ...
                pipeline_s0_s1_sequence=pipeline_s0_s1_sequence,
                thr_tmem_load=thr_tmem_load,
                tmem_load_red=tmem_load_red,
                tStS_t2r_red=tStS_t2r_red,
                tScS_t2r_red=tScS_t2r_red,
    ...
    def softmax_step(
        ...
        tmem_load_red: Optional[cute.TiledCopy],
        tStS_t2r_red: Optional[cute.Tensor],
        tScS_t2r_red: Optional[cute.Tensor],
        thr_tmem_store: cute.CopyAtom,
        thr_tmem_store_scale: cute.CopyAtom,
        tStS_t2r: cute.Tensor,
        tScScale: cute.Tensor,
        ...
        if const_expr(tmem_load_red is not None and mask_fn is None):
            tSrS_t2r_red = cute.make_fragment_like(tScS_t2r_red, self.qk_acc_dtype)
            tSrS_t2r_max = cute.make_rmem_tensor(
                cute.make_layout(
                    (1, tScS_t2r_red.shape[1], tScS_t2r_red.shape[2])
                ),
                self.qk_acc_dtype,
            )
            cute.copy(
                tmem_load_red,
                tStS_t2r_red,
                (tSrS_t2r_red, tSrS_t2r_max),
            )
            tSrS_t2r = tSrS_t2r_red
            hw_row_max = tSrS_t2r_max[0]
            for i in cutlass.range_constexpr(1, cute.size(tSrS_t2r_max)):
                hw_row_max = cute.arch.fmax(hw_row_max, tSrS_t2r_max[i])
        else:
            ...
            hw_row_max = None
        if const_expr(self.blockscaled and self.q_stage == 2):
            if stage == 0:
        ...
        if const_expr(hw_row_max is not None and mask_fn is None):
        ...
        while work_tile.is_valid_tile:
            ...
            if const_expr(mLSE is not None):
                if const_expr(not seqlen.has_cu_seqlens_q):
                    ...
                    offset = (
                        seqlen.offset_o if const_expr(not self.pack_gqa) else (0, seqlen.offset_o)
                    )
                    if const_expr(self.is_split_kv):
    ...
    def load_Q(
        ...
        sf_block_base: Int32 = 0,
    ):
        pipeline_q.producer_acquire_w_index_phase(stage, phase)
        barrier = pipeline_q.sync_object_full.get_barrier(stage)
        load_Q_fn(src_idx=block, dst_idx=stage, tma_bar_ptr=barrier)
        if const_expr(self.external_mx):
            cute.copy(
                tma_atom_SFQ,
                tSFQgSFQ[None, sf_block_base + block],
                tSFQsSFQ[None, stage],
                tma_bar_ptr=barrier,
            )
    ...
    def load_KV(
        ...
        if const_expr(self.use_tma_KV):
            ...
            barrier = pipeline_kv.producer_get_barrier(producer_state)
            cute.copy(tma_atom, tXgX_cur, tXsX_cur, tma_bar_ptr=barrier)
            if const_expr(self.external_mx):
                cute.copy(
                    tma_atom_SF,
                    tSFgSF[None, block],
    ...
    def apply_score_mod(
        ...
        if cutlass.const_expr(aux_data is not None and aux_data.tensors is not None):
