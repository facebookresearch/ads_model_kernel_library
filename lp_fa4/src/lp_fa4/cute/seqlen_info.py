...
class SeqlenInfoQK:
    ...
    broadcast_q: cutlass.Constexpr[bool]
    offset_o: Int32
    padded_offset_o: Int32
    ...
    def create(
        ...
        if const_expr(broadcast_q):
            assert mCuSeqlensQ is not None
            offset_q = Int32(0)
            padded_offset_q = Int32(0)
            offset_sf_q = Int32(0)
            seqlen_q = mCuSeqlensQ[1] - mCuSeqlensQ[0]
            offset_o = batch_idx * seqlen_q
            padded_offset_o = cute.assume(
                (offset_o + batch_idx * tile_m) // tile_m * tile_m,
                divby=tile_m,
            )
        else:
            ...
            offset_sf_q = (
                offset_q
                if const_expr(mCuSeqlensSFQ is None)
            ...
            offset_o = offset_q
            padded_offset_o = padded_offset_q
        ...
        offset_sf_k = (
            offset_k if const_expr(mCuSeqlensSFK is None) else mCuSeqlensSFK[batch_idx]
        )
        ...
        return SeqlenInfoQK(
            ...
            offset_o=offset_o,
            padded_offset_o=padded_offset_o,
        )
    ...
    def offset_batch_O(
        ...
        if const_expr(not self.broadcast_q):
            return self.offset_batch_Q(mO, batch_idx, dim, padded, ragged)
        offset_o = self.offset_o if const_expr(not padded) else self.padded_offset_o
        if const_expr(not ragged):
            offset = offset_o if const_expr(cute.rank(mO.shape[0]) == 1) else (None, offset_o)
            idx = (offset,) + (None,) * (cute.rank(mO) - 1)
            return cute.domain_offset(idx, mO)
        if const_expr(cute.rank(mO.shape[0]) == 1):
            return copy_utils.offset_ragged_tensor(
                mO, offset_o, self.seqlen_q, ragged_dim=0, ptr_shift=True
            )
        assert cute.rank(mO.shape[0]) == 2
        idx = ((None, None),) + (None,) * (cute.rank(mO) - 1)
        mO = mO[idx]
        mO = copy_utils.offset_ragged_tensor(
            mO, offset_o, self.seqlen_q, ragged_dim=1, ptr_shift=True
        )
        return cute.group_modes(mO, 0, 2)
