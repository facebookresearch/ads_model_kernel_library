...
from ads_mkl.ops.cute_dsl.gdpa.src.interface import flash_attn_func as flash_attn_func_python
from ads_mkl.ops.cute_dsl.gdpa.src.interface import flash_attn_varlen_func as flash_attn_varlen_func_python


from triton.testing import do_bench

def time_fwd(func, *args, warmup=100, rep=1000, verbose=True, desc="", **kwargs):
    return Timing(do_bench(lambda: func(*args, **kwargs), warmup=warmup, rep=rep) * 1e-3)
    ...
    for varlen in [True, False]: # 
        ...
        for causal in [False]:
            ...
            if flash_attn_func_python is not None:
                if not varlen:
                    m1_py = time_fwd(flash_attn_func_python, q, k, v_fa3, causal=causal, window_size=window_size, learnable_sink=sinks, softcap=softcap, pack_gqa=pack_gqa, verbose=verbose, desc='Fav3 python')
                    print(f"### {q.shape = } {k.shape = } {v.shape = } ###")
                else:
                    m1_py = time_fwd(flash_attn_varlen_func_python, q_unpad, k_unpad, v_unpad, cu_seqlens_q, cu_seqlens_k, max_seqlen_q=seqlen_q, page_table=None, causal=causal, window_size=window_size, softcap=softcap, pack_gqa=pack_gqa, verbose=verbose, desc='Fav3 python')
                    print(f"### {q_unpad.shape = } {k_unpad.shape = } {v_unpad.shape = } ###")
