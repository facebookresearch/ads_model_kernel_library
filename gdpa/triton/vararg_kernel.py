...
# Credit: this file is largely copied from fair/xformers/xformers/triton/vararg_kernel.py
# with some modifications to make it work with our use case and avoid extra build dependency of xformer.
...
class _ForLoopUnroller(ast.NodeTransformer):
    ...
    def visit_Subscript(self, node):
        # Pattern-matching `value[slice][slice]`
...
class _VisitorVarargKernel(ast.NodeTransformer):
    def __init__(self, N, unroll_as_const):
        self.inline_variables = set()
        self.N = N
        self.unroll_as_const = unroll_as_const
...
def unroll_varargs(
    kernel, N: int, mode: VarargModes = VarargModes.UNROLL, unroll_as_const=False
):
    """
    ...
    if mode == VarargModes.UNROLL:
        nodeVisitor = _VisitorUnrollKernel(N=N, unroll_as_const=unroll_as_const)
    elif mode == VarargModes.CONDITIONAL:
        nodeVisitor = _VisitorConditionalKernel(N=N, unroll_as_const=unroll_as_const)
