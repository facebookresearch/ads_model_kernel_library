...
def cvt_fp8x4_f32(
    ...
    *,
    ...
    """Convert 4 F32 values to 4 FP8 values packed in an Int32.

    Uses PTX cvt.rn.satfinite.e4m3x2.f32 or cvt.rn.satfinite.e5m2x2.f32.
    """
    assert to_dtype in [
        cutlass.Float8E4M3FN,
        cutlass.Float8E5M2,
    ], "to_dtype must be Float8E4M3FN or Float8E5M2"

    fp8_type = "e4m3" if to_dtype is cutlass.Float8E4M3FN else "e5m2"

    # Convert 4 F32 to 4 FP8: pack pairs into 16-bit values, then combine into 32-bit
    return cutlass.Int32(
        llvm.inline_asm(
            ...
            f"""{{
                .reg .b16 lo, hi;
                cvt.rn.satfinite.{fp8_type}x2.f32 lo, $2, $1;
                cvt.rn.satfinite.{fp8_type}x2.f32 hi, $4, $3;
                mov.b32 $0, {{lo, hi}};
            }}""",
            "=r,f,f,f,f",
            has_side_effects=False,
...
def cvt_f16(src: cute.Tensor, dst_or_dtype):
    """Convert Float32 tensor to Float16/BFloat16/Float8.

    Args:
        src: Source tensor with Float32 element type
        dst_or_dtype: Either a destination tensor or a dtype (Float16/BFloat16/Float8E4M3FN/Float8E5M2)
    ...
    if const_expr(isinstance(dst_or_dtype, type)):
        ...
        if const_expr(dst.element_type in [cutlass.BFloat16, cutlass.Float16]):
            # FP16/BF16 path: pack 2 F32 into 1 i32 (2 x 16-bit)
            assert cute.size(src.shape) % 2 == 0, (
                "src must have an even number of elements for FP16/BF16"
            )
        ...
        elif const_expr(dst.element_type in [cutlass.Float8E4M3FN, cutlass.Float8E5M2]):
            # FP8 path: pack 4 F32 into 1 i32 (4 x 8-bit)
            assert cute.size(src.shape) % 4 == 0, (
                "src must have a multiple of 4 elements for FP8"
            )
            dst_i32 = cute.recast_tensor(dst, cutlass.Int32)
            assert cute.size(dst_i32.shape) * 4 == cute.size(src.shape)
            for i in cutlass.range_constexpr(cute.size(dst_i32)):
                dst_i32[i] = cvt_fp8x4_f32(
                    src[4 * i],
                    src[4 * i + 1],
                    src[4 * i + 2],
                    src[4 * i + 3],
            ...
            assert False, "dst must be BFloat16, Float16, Float8E4M3FN, or Float8E5M2"
