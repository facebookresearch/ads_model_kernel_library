...
import ads_mkl.ops.oss.gdpa.src.ampere_helpers as sm80_utils
import ads_mkl.ops.oss.gdpa.src.copy_utils as copy_utils
import ads_mkl.ops.oss.gdpa.src.hopper_helpers as sm90_utils
...
class FlashAttentionBackwardPostprocess:
    ...
    def __call__(
        ...
        mMaxSeqlenQ: cutlass.Int32 | int | None = None,
    ):
        ...
        if mMaxSeqlenQ is not None:
            mMaxSeqlenQ = cutlass.Int32(mMaxSeqlenQ)
        ...
        if const_expr(mCuSeqlensQ is not None):
            TileScheduler = SingleTileVarlenSchedulerSimple
    ...
    def kernel(
        ...
        if work_tile.is_valid_tile:
            ...
            rdQ.store((acc.load()).to(self.dtype))
            ...
            tdQgdQ = gmem_thr_copy_dQ.partition_D(gdQ)  # was partition_S
            tdQsdQ = gmem_thr_copy_dQ.partition_S(sdQ)  # was partition_D
