...
import ads_mkl.ops.oss.gdpa.src.barrier as barrier
import ads_mkl.ops.oss.gdpa.src.copy_utils as copy_utils
import ads_mkl.ops.oss.gdpa.src.pipeline as pipeline
import ads_mkl.ops.oss.gdpa.src.utils as utils
import cuda.bindings.driver as cuda
import cutlass
import cutlass.cute as cute
import cutlass.utils.blackwell_helpers as sm100_utils_basic
from ads_mkl.ops.oss.gdpa.src.activation_utils import Gelu, Relu
from ads_mkl.ops.oss.gdpa.src.blackwell_helpers import (  # noqa
    gemm_ptx_w_idx,
    gemm_w_idx,
)
from ads_mkl.ops.oss.gdpa.src.block_info import BlockInfo
from ads_mkl.ops.oss.gdpa.src.mask import AttentionMask
from ads_mkl.ops.oss.gdpa.src.named_barrier import NamedBarrierBwdSm100
from ads_mkl.ops.oss.gdpa.src.seqlen_info import SeqlenInfoQK
from ads_mkl.ops.oss.gdpa.src.tile_scheduler import (
    ParamsBase,
    SingleTileLPTScheduler,
    SingleTileScheduler,
    SingleTileVarlenScheduler,
    SingleTileVarlenSchedulerPersistent,
    SingleTileVarlenSchedulerSimple,
...
class FlashAttentionBackwardSm100:
    ...
    def __init__(
        ...
        assert qhead_per_kvhead == 1, (
            "GQA is not supported yet in FlashAttentionBackwardSm100"
        )
        ...
        self.kq_acc_dtype = self.vdo_acc_dtype = self.pdo_acc_dtype = (
            self.dsq_acc_dtype
    ...
    def _setup_smem_layout(self):
        ...
        self.sdQaccum_tile = (self.tile_m, self.dQ_reduce_ncol)
        self.sdQaccum_layout = sm100_utils_basic.make_smem_layout_epi(
            self.dq_dtype,
    ...
    def __call__(
        ...
        mMaxSeqlenK: Int32 | int | None = None,
        ...
        # For varlen KV, both TMA store with reduce add and non-TMA store have correct accuracy.
        # On smaller Q length, it seems TMA store is faster, on larger, it seems non-TMA store is faster.
        # self.use_tma_store = (
        #     self.use_tma_store and mCuSeqlensK is None and mSeqUsedK is None
        # )
        self.q_dtype = mQ.element_type
        self.k_dtype = mK.element_type
        self.v_dtype = mV.element_type
        self.do_dtype = mdO.element_type
        # TODO Debug TMA store for FP8
        self.use_tma_store = self.v_dtype.width != 8
        ...
        new_stride = (
            lambda t: (  # noqa
                *(
                    cute.assume(
                        s, divby=self.universal_copy_bits // t.element_type.width
                    )
                    for s in t.stride[:-1]
                ),
                t.stride[-1],
        ...
        # (b, s, n, h) --> (s, h, n, b) or (b*s, n, h) -> (b*s, h, n)
        layout_transpose = (
            [1, 3, 2, 0] if const_expr(mCuSeqlensQ is None) else [0, 2, 1]
        )
        mQ, mK, mV, mdO, mdK, mdV, mdQ = [
            utils.select(t, mode=layout_transpose)
            for t in (mQ, mK, mV, mdO, mdK, mdV, mdQ)
        ]
        # LSE_dPsum_dQaccum_transpose = [2, 1, 0]  # (b, n, s) --> (s, n, b)
        # (b, n, s*h) --> (s*h, n, b) or (n, b*s*h) -> (b*s*h, n)
        LSE_dPsum_dQaccum_transpose = (
            [2, 1, 0] if const_expr(mCuSeqlensQ is None) else [1, 0]
        )
        mLSE, mdPsum = [
            utils.select(t, mode=LSE_dPsum_dQaccum_transpose) for t in (mLSE, mdPsum)
        ]
        # dO_transpose = [1, 0, 2, 3]
        # (s, h, n, b) -> (h, s, n, b) or (b*s, h, n) -> (h, b*s , n)
        dO_transpose = [1, 0, 2, 3] if const_expr(mCuSeqlensK is None) else [1, 0, 2]
        ...
        if const_expr(mMaxSeqlenK is not None):
            mMaxSeqlenK = Int32(mMaxSeqlenK)
        ...
        if const_expr(self.use_tma_store):
            if const_expr(self.dk_dtype.width == 32):
                ...
                # Change from CopyBulkTensorTileS2GOp to CopyReduceBulkTensorTileS2GOp
                # to handle KV length not a multiple of tiler
                tma_copy_op_dKV = cpasync.CopyReduceBulkTensorTileS2GOp()
        ...
        tma_atom_dQ, mdQ_tma_tensor = cpasync.make_tiled_tma_atom(
        ...
        val_layout_r2s_dKV = cute.make_ordered_layout(
            (1, self.universal_copy_bits // self.dk_dtype.width), order=(1, 0)
        ...
        tma_atom_Q, tma_tensor_Q = cute.nvgpu.make_tiled_tma_atom_B(
            (
                tma_load_op
                if const_expr(self.cluster_shape_mnk[1] == 1)
                else tma_load_op_multicast
            ),
            mQ,
        ...
        #     self.tile_m * self.dQ_reduce_ncol * self.dq_dtype.width // 8
        # )
        ...
        @cute.struct
        class SharedStorage:
            Q_mbar_ptr: cute.struct.MemRange[cutlass.Int64, 2 * self.Q_stage]  # 0, 4
            dO_mbar_ptr: cute.struct.MemRange[cutlass.Int64, 2 * self.dO_stage]  # 4, 6
            ...
            S_mbar_ptr: cute.struct.MemRange[cutlass.Int64, 2 * 1]  # 6, 8
            dP_mbar_ptr: cute.struct.MemRange[cutlass.Int64, 2 * 1]  # 8, 10
            dS_mbar_ptr: cute.struct.MemRange[cutlass.Int64, 2 * 1]  # 10, 12
            dKV_mbar_ptr: cute.struct.MemRange[cutlass.Int64, 2 * 2]  # 12, 16
            dQ_mbar_ptr: cute.struct.MemRange[cutlass.Int64, 2]  # 16, 18
            ...
            sdQaccum: cute.struct.Align[
                cute.struct.MemRange[self.dq_dtype, cute.cosize(self.sdQaccum_layout)],
                self.buffer_align_bytes,
            ]
    ...
    def kernel(
        ...
        mdQ_tma_tensor: Optional[cute.Tensor],
        ...
        sdQaccum = storage.sdQaccum.get_tensor(
            sdQaccum_layout.outer, swizzle=sdQaccum_layout.inner, dtype=self.dq_dtype
        )
        ...
        # S
        # Note: the `cta_mma_` prefix comes from the fact that in B200, it uses UMMA which
        # repurposed all "thread layouts" as "layouts of the CTAs" collaborating on an MMA instruction.
        # This is also why it's always slicing by zero here as we have only one CTA.
        cta_mma_SdP = tiled_mma_SdP.get_slice(0)
        Sacc_shape = cta_mma_SdP.partition_shape_C(self.mma_tiler_kq[:2])  # (M, N)
        tStS = cta_mma_SdP.make_fragment_C(Sacc_shape)
        # (MMA, MMA_M, MMA_N)
        tStS = cute.make_tensor(tmem_ptr + self.tmem_S_offset, tStS.layout)
        # dP
        dPacc_shape = cta_mma_SdP.partition_shape_C(self.mma_tiler_vdo[:2])
        tdPtdP = cta_mma_SdP.make_fragment_C(dPacc_shape)
        tdPtdP = cute.make_tensor(tmem_ptr + self.tmem_dP_offset, tdPtdP.layout)
        # dV
        cta_mma_dV = tiled_mma_dV.get_slice(0)
        dvacc_shape = cta_mma_dV.partition_shape_C(self.mma_tiler_pdo[:2])
        tdVtdV = cta_mma_dV.make_fragment_C(dvacc_shape)
        tdVtdV = cute.make_tensor(tmem_ptr + self.tmem_dV_offset, tdVtdV.layout)
        tP_ptr = cute.make_ptr(
            self.do_dtype, self.tmem_P_offset, cute.AddressSpace.tmem
        )
        tP = cute.make_tensor(tP_ptr, tP_layout.outer)
        # dK
        cta_mma_dK = tiled_mma_dK.get_slice(0)
        dkacc_shape = cta_mma_dK.partition_shape_C(self.mma_tiler_dsq[:2])
        tdKtdK = cta_mma_dK.make_fragment_C(dkacc_shape)
        tdKtdK = cute.make_tensor(tmem_ptr + self.tmem_dK_offset, tdKtdK.layout)
        # dQ
        cta_mma_dQ = tiled_mma_dQ.get_slice(0)
        dQacc_shape = cta_mma_dQ.partition_shape_C(self.mma_tiler_dsk[:2])
        tdQtdQ = cta_mma_dQ.make_fragment_C(dQacc_shape)
    ...
    def load(
        self,
        cta_mma_SdP: cute.core.ThrMma,
        cta_mma_dV: cute.core.ThrMma,
        mQ: cute.Tensor,
        mK: cute.Tensor,
        mV: cute.Tensor,
        mLSE: cute.Tensor,
        ...
        while work_tile.is_valid_tile:
            ...
            if const_expr(not seqlen.has_cu_seqlens_q):
                ...
                mdO_cur = cute.domain_offset((0, offset), mdO[None, None, head_idx])
                # mLSE_cur = cute.domain_offset((offset,), mLSE[None, head_idx])
                # mPsum_cur = cute.domain_offset((offset,), mdPsum[None, head_idx])
            ...
            tdVgdO = cta_mma_dV.partition_B(gdO)
            ...
            # copy_stats(
            #     gLSE[None, m_block_min], sLSE[None, 0], mbar_ptr=LSE_full_mbar_ptr
            # )
            # V & dO
            ...
            #     )
            #     load_dPsum(
            #         src_idx=m_block_min, dst_idx=0, tma_bar_ptr=dPsum_full_mbar_ptr
            #     )
            # copy_stats(
            #     gdPsum[None, m_block_min],
            #     sdPsum[None, 0],
            #     mbar_ptr=dPsum_full_mbar_ptr,
            # )
            ...
            for m_block in cutlass.range(m_block_min + 1, m_block_max, unroll=1):
                ...
                # copy_stats(
                #     gLSE[None, m_block], sLSE[None, 0], mbar_ptr=LSE_full_mbar_ptr
                # )
                # dO
                ...
                #     dPsum_empty_mbar_ptr, dpsum_empty_consumer_phase
                # )
                ...
                #     )
                #     load_dPsum(
                #         src_idx=m_block, dst_idx=0, tma_bar_ptr=dPsum_full_mbar_ptr
                #     )
                # copy_stats(
                #     gdPsum[None, m_block],
                #     sdPsum[None, 0],
    ...
    def mma(
        ...
        # )
        mma_pdo_fn = partial(
            gemm_w_idx, tiled_mma_dV, tdVtdV, tdVrP, tdVrdO, A_idx=None
        )
        # mma_pdo_fn = partial(
        ...
        while work_tile.is_valid_tile:
            ...
            pipeline_dQ.sync_object_empty.wait(0, dQ_producer_phase_acc)
            ...
            for _ in cutlass.range(m_block_min + 1, m_block_max, unroll=1):
                ...
                dQ_producer_phase_acc ^= 1

                # 3) dK = dS.T @ Q
                mma_dsq_fn(
                    B_idx=consumer_state_Q_prev.index, zero_init=not accumulate_dK
                )
    ...
    def compute_loop(
        ...
        tScS = cta_mma_SdP.partition_C(cute.make_identity_tensor(self.mma_tiler_kq[:2]))
        ...
        assert self.v_dtype.width in [8, 16], "Only support 8/16-bit currently"
        # Repetition depends on dtype width: 16 for bf16/fp16 (16-bit), 8 for fp8 (8-bit)
        ...
        # ((32, 1), 2, 1, 1, STAGE)
        # tSsLSE = thr_copy_t2r.partition_D(cta_mma_SdP.partition_C(sLSE_2D))
        # tSsdPsum = thr_copy_t2r.partition_D(cta_mma_SdP.partition_C(sdPsum_2D))
        # rmem -> tmem
        ...
        while work_tile.is_valid_tile:
            ...
            for m_block in cutlass.range(m_block_min, m_block_max, unroll=1):
                ...
                # tSrP_r2t = tSrP_r2t_f32
                tSrP_r2t = cute.recast_tensor(tSrP_r2t_f32, self.q_dtype)
                tSrS_t2r_tmp = cute.make_fragment(tScS_t2r.shape, Float32)
                for stage in cutlass.range_constexpr(
                    cute.size(tSrS_t2r, mode=[1]), unroll=1
                ):
                    tSrS_cur = tSrS_t2r[None, stage, 0, 0]
                    # tSrP_cur = tSrP_r2t_f32[None, stage, 0, 0]
                    tSrS_tmp_cur = tSrS_t2r_tmp[None, stage, 0, 0]
                    ...
                    # ):
                    #     tSrS_tmp_cur[2 * v], tSrS_tmp_cur[2 * v + 1] = gelu.fast_gelu(
                    #         (tSrS_cur[2 * v], tSrS_cur[2 * v + 1])
                    #     )
                    #     tSrS_cur[2 * v], tSrS_cur[2 * v + 1] = gelu.grad_fast_gelu(
                    #         (tSrS_cur[2 * v], tSrS_cur[2 * v + 1])
                    #     )
                    if const_expr(self.activation == "relu"):
                        relu.grad_relu_and_convert(
                            tSrS_cur,
                            tSrS_cur,
                            tSrS_tmp_cur,
                    ...
                    # utils.cvt_f16(tSrP_cur, tSrP_cur)
                    # utils.cvt_f16(tSrS_tmp_cur, tSrP_r2t[None, stage, 0, 0])
                    # tSrP_r2t = cute.make_tensor(tSrP_r2t.iterator, tScP_r2t.shape)
    ...
    def dQacc_reduce(
        ...
        thr_layout_r2s_dKV = cute.make_ordered_layout(
            (num_reduce_threads, 1), order=(1, 0)
        )  # 128 threads
        val_layout_r2s_dKV = cute.make_ordered_layout(
            (1, self.universal_copy_bits // self.dq_dtype.width), order=(1, 0)
        )  # 8 elements
        copy_atom_r2s_dKV = cute.make_copy_atom(
            cute.nvgpu.CopyUniversalOp(),
            self.dq_dtype,
        ...
        thr_copy_dQaccum_r2s = cute.make_tiled_copy_tv(
            copy_atom_r2s_dKV, thr_layout_r2s_dKV, val_layout_r2s_dKV
        ).get_slice(tidx)

        tdQsdQ_r2s_p = thr_copy_dQaccum_r2s.partition_D(sdQaccum)
        cdQ = cute.make_identity_tensor((128, 32))
        tdQcdQ = thr_copy_dQaccum_r2s.partition_S(cdQ)
        ...
        while work_tile.is_valid_tile:
            ...
            m_block_min, m_block_max = block_info.get_m_block_min_max(seqlen, n_block)
            if const_expr(not seqlen.has_cu_seqlens_q):
                mdQ_cur = mdQ_tma_tensor[None, None, head_idx, batch_idx]
            else:
                mdQ_cur = cute.domain_offset(
                    (seqlen.offset_q, 0), mdQ_tma_tensor[None, None, head_idx]
                )
            ...
            gdQaccum = cute.flat_divide(
                ...
                (
                    self.tile_m,
                    self.tile_hdim // self.dQaccum_reduce_stage,
                ),
            )
            ...
            for m_block in cutlass.range(m_block_min, m_block_max, unroll=1):
                ...
                tdQrdQ_t2r_casted = cute.make_fragment(tdQrdQ_t2r_shape, self.dq_dtype)
                # Use cvt_f16 for FP8 conversion instead of generic .to() which crashes on Blackwell
                utils.cvt_f16(tdQrdQ_t2r, tdQrdQ_t2r_casted)
                tdQrdQ_t2r = tdQrdQ_t2r_casted
                cute.arch.fence_view_async_tmem_load()
                cute.arch.sync_warp()
                with cute.arch.elect_one():
                ...
                def tma_store_fn(
                    src_idx, dst_idx, tma_atom_dQ, tdQsdQ, tdQgdQ, m_block
                ):
                ...
                for stage in cutlass.range_constexpr(
                    ...
                    tdQsdQ_r2s = tdQsdQ_r2s_p[None, None, None, smem_idx]
                    tdQrdQ_r2s = cute.make_tensor(
                        tdQrdQ_t2r[None, stage, None, None].iterator, tdQcdQ.shape
                    )
                    ...
                    if const_expr(not delay_tma_store):
                        tma_store_fn(
                            smem_idx, stage, tma_atom_dQ, tdQsdQ, tdQgdQ, m_block
                        )
                ...
                if const_expr(delay_tma_store):
                    tma_store_fn(
                        smem_idx_prev, stage_prev, tma_atom_dQ, tdQsdQ, tdQgdQ, m_block
                    )
    ...
    def epilogue_dKV(
        ...
        head_idx_kv = head_idx // self.qhead_per_kvhead
        if const_expr(not seqlen.has_cu_seqlens_k):
            mdK_cur = mdK[None, None, head_idx_kv, batch_idx]
            mdV_cur = mdV[None, None, head_idx_kv, batch_idx]
        else:
            mdK_cur = cute.domain_offset(
                (seqlen.offset_k, 0), mdK[None, None, head_idx_kv]
            )
            mdV_cur = cute.domain_offset(
                (seqlen.offset_k, 0), mdV[None, None, head_idx_kv]
            )
        ...
        tdVcdV = cta_mma_dV.partition_C(cdV)
        ...
        tdVgdV = cta_mma_dV.partition_C(gdV_tile)
        tdVgdV_r2g_p = thr_tmem_ld_dV.partition_D(tdVgdV)
        tdVgdV_r2g = self.split_wg(tdVgdV_r2g_p, wg_idx, num_wg)

        row_limit = seqlen.seqlen_k - n_block * self.tile_n
        for i in cutlass.range_constexpr(cute.size(tdVrdV_r2s, mode=[1])):
            if tdVcdV_t2r[0, i, 0, 0][0] < row_limit:
                cute.copy(
                    tiled_gmem_store_dV,
                    tdVrdV_r2s[None, i, None, None],
                    tdVgdV_r2g[None, i, None, None],
                )
        ...
        tdKcdK = cta_mma_dK.partition_C(cdK)
        ...
        for i in cutlass.range_constexpr(cute.size(tdKrdK_t2r, mode=[1])):
            dK_vec = tdKrdK_t2r[(None, i, 0, 0)].load()
        ...
        tdKgdK = cta_mma_dK.partition_C(gdK_tile)
        tdKgdK_r2g_p = thr_tmem_ld_dK.partition_D(tdKgdK)
        tdKgdK_r2g = self.split_wg(tdKgdK_r2g_p, wg_idx, num_wg)

        for i in cutlass.range_constexpr(cute.size(tdKrdK_r2s, mode=[1])):
            if tdKcdK_t2r[0, i, 0, 0][0] < row_limit:
                cute.copy(
                    tiled_gmem_store_dK,
                    tdKrdK_r2s[None, i, None, None],
                    tdKgdK_r2g[None, i, None, None],
                )
    ...
    def epilogue_dK_or_dV_tma(
        ...
        if const_expr(not seqlen.has_cu_seqlens_k):
            ...
            mdKV_cur = cute.domain_offset(
                (seqlen.offset_k, 0), mdKV[None, None, head_idx_kv]
            )
