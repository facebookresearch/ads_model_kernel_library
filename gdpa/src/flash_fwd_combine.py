...
from ads_mkl.ops.oss.gdpa.src.fast_math import FastDivmod
from ads_mkl.ops.oss.gdpa.src.seqlen_info import SeqlenInfo
...
class FlashAttentionForwardCombine:
    ...
    @cute.jit
    def _compute_final_lse_and_scales(
        self,
        ts2rrLSE: cute.Tensor,
        ts2rcLSE: cute.Tensor,
        ts2rsLSE: cute.Tensor,
        sMaxValidSplit: cute.Tensor,
        ...
        return lse_sum, max_valid_split

    @cute.jit
    def _setup_and_load_o_partial_initial(
        self,
        tidx: Int32,
        m_block: Int32,
        k_block: Int32,
    ...
    @cute.jit
    def _accumulate_partial_results(
        self,
        sMaxValidSplit: cute.Tensor,
        sLSE: cute.Tensor,
    ...
    def kernel(
        ...
        if (const_expr(num_splits_dynamic_ptr is None) or num_splits > 1) and (
            ...
            self._load_lse_partial_to_smem(
                tidx,
                m_block,
                batch_idx,
                sLSE,
            ...
            (
                ...
                tOsO_partial,
                num_rows,
            ) = self._setup_and_load_o_partial_initial(
                tidx,
                m_block,
                k_block,
                batch_idx,
            ...
            lse_sum, max_valid_split = self._compute_final_lse_and_scales(
                ts2rrLSE, ts2rcLSE, ts2rsLSE, sMaxValidSplit, s2r_tiled_copy_LSE
            )
            ...
            tOrO = self._accumulate_partial_results(
                sMaxValidSplit,
                sLSE,
                tOsO_partial,
                tOcO,
