...
class AttentionMask:
    ...
    def apply_mask_sm100_transposed(
        ...
        ROW = 0 if const_expr(not self.swap_AB) else 1
        COL = 1 if const_expr(not self.swap_AB) else 0
        seqlenq_offset = tScS_t2r[0][1]
        seqlenq_col_limit = self.seqlen_q - m_block * self.tile_m
        seqlenk_col_limit = self.seqlen_k - n_block * self.tile_n
        if const_expr(not mask_causal and not mask_local):
            if const_expr(mask_seqlen):
                r2p = True
                if const_expr(not r2p):
                    for i in cutlass.range(cute.size(tScS_t2r.shape), unroll_full=True):
                        acc_S[i] = (
                            Float32(0)
                            if tScS_t2r[i][1] >= seqlenq_col_limit
                            else acc_S[i]
                        )
                        acc_S[i] = (
                            Float32(0)
                            if tScS_t2r[i][0] >= seqlenk_col_limit
                            else acc_S[i]
                        )
                else:
                    num_rep = cute.size(tScS_t2r, mode=[0])
                    mask_r2p_transposed_inbound_check(
                        acc_S, seqlenq_col_limit - seqlenq_offset, num_rep
                    )
                    if tScS_t2r[0][0] >= seqlenk_col_limit:
